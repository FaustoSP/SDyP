...
int main(int argc, char* argv[]){
    ...
    while(!convergio){
        ...
        mSecConvergido[0] = ((mSec[0] + mSec[1] + mSec[N] + mSec[N+1]) / 4.0); //Esquina superior izquierda (0,0).
        mSecConvergido[N-1] = ((mSec[(N-2*N)] + mSec[(N-1)*N] + mSec[(N-2)*N+1] + mSec[(N-1)*N+1]) / 4.0); //Esquina superior derecha (0,N-1).
        mSecConvergido[(N-1)*N] = ((mSec[N-2] + mSec[N+N-2] + mSec[N-1] + mSec[N+N-1]) / 4.0); //Esquina inferior izquierda (N-1,0).
        mSecConvergido[(N-1)*N+N-1] = ((mSec[(N-2)*N+N-2] + mSec[(N-1)*N+N-2] + mSec[(N-2)*N+N-1] + mSec[(N-1)*N+N-1]) / 4.0); //Esquina inferior derecha (N-1,N-1)

        //Luego se calcula el valor de los bordes, menos las esquinas.
        //Borde superior, 1<i<N y j=0
        for(i=1;i<N-1;i++){
            mSecConvergido[i*N] = ((mSec[(i-1)*N] + mSec[i*N] + mSec[(i+1)*N] + mSec[(i-1)*N+1] + mSec[i*N+1] + mSec[(i+1)*N+1]) / 6.0);
        }
        //Borde inferior, 1<i<N y j=N-1
        for(i=1;i<N-1;i++){
            mSecConvergido[i*N+N-1] = ((mSec[(i-1)*N+N-2] + mSec[i*N+N-2] + mSec[(i+1)*N+N-2] + mSec[(i-1)*N+N-1] + mSec[i*N+N-1] + mSec[(i+1)*N+N-1]) / 6.0);
        }
        //Borde izquierdo, i=0, 1<j<N
        for(j=1;j<N-1;j++){
            mSecConvergido[j] = ((mSec[j-1] + mSec[j] + mSec[j+1] + mSec[N+j-1] + mSec[N+j] + mSec[N+j+1]) / 6.0);
        }
        //Borde derecho, i=N-1, 1<j<N
        for(j=1;j<N-1;j++){
            mSecConvergido[(N-1)*N+j] = ((mSec[(N-2)*N+j-1] + mSec[(N-2)*N+j] + mSec[(N-2)*N+j+1] + mSec[(N-1)*N+j-1] + mSec[(N-1)*N+j] + mSec[(N-1)*N+j+1]) / 6.0);
        }
        ...
        //Se evalúa todos los valores del vector convergieron.
        convergio = true;

...
int main(int argc, char* argv[]){
    ...
    while(!convergio){
        ...
        mSecConvergido[0] = ((mSec[0] + mSec[1] + mSec[N] + mSec[N+1]) *a4); //Esquina superior izquierda (0,0).
        mSecConvergido[N-1] = ((mSec[N-1] + 
                                mSec[N-2] + 
                                mSec[(2*N)-1] + 
                                mSec[(2*N)-2]) *a4); //Esquina superior derecha (0,N-1).
        mSecConvergido[(N-1)*N] = ((mSec[(N-1)*N] + 
                                    mSec[(N-1)*N+1] + 
                                    mSec[(N-2)*N] + 
                                    mSec[(N-2)*N+1]) * a4); //Esquina inferior izquierda (N-1,0).
        mSecConvergido[N*N -1] = ((mSec[N*N -1] + 
                                    mSec[N*N -2] + 
                                    mSec[(N-1)*N -1] + 
                                    mSec[(N-1)*N -2]) * a4); //Esquina inferior derecha (N-1,N-1)

        //Luego se calcula el valor de los bordes, menos las esquinas.
        for(j=1;j<N-1;j++){
            //superior
            mSecConvergido[j]=(mSec[j]+mSec[j-1]+mSec[j+1]+mSec[j+N]+mSec[j+N+1]+mSec[j+N-1])*a6;
            //inferior
            mSecConvergido[N*(N-1)+j]=(mSec[N*(N-1)+j] + mSec[N*(N-1)+j-1] + mSec[N*(N-1)+j+1] +
                mSec[N*(N-2)+j] + mSec[N*(N-2)+j+1] + mSec[N*(N-2)+j-1])*a6;    
        }

        for(i=1;i<N-1;i++){
            //izquierda
            mSecConvergido[i*N] = (mSec[i*N] + mSec[i*N+1] +
                mSec[(i-1)*N] + mSec[(i-1)*N+1] +
                mSec[(i+1)*N] + mSec[(i+1)*N+1])*a6;
            //derecha
            mSecConvergido[i*N+(N-1)] = (mSec[i*N+(N-1)] + mSec[i*N+(N-1)-1] +
                mSec[(i-1)*N+(N-1)] + mSec[(i-1)*N+(N-1)-1] +
                mSec[(i+1)*N+(N-1)] + mSec[(i+1)*N+(N-1)-1])*a6;
        }
